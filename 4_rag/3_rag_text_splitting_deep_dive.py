...
"""
splits text into chunks based on specified number of characters.
...
"""
Until now we've created vector store for diff type of text splitters.
Now we are going to query a vector store that is created above.
"""
...
def query_vector_store(store_name, query):
    ...
    if os.path.exists(persistant_dir):
        ...
        db = Chroma(persist_directory=persistant_dir,
        ...
        for i, doc in enumerate(relevant_docs, 1):
            print(f"Document {i}: \n{doc.page_content}\n")
            if doc.metadata:
                print(f"Souce :{doc.metadata.get('source','unknown')}\n")
    else:
